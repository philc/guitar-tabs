...
async function processPages(files) {
  ...
  const resources = ["print.css", "styles.css"];
...
async function createIndex(files) {
  ...
  htmlStr = await wrapInLayout(htmlStr, "Guitar Tabs");
  // Label this as the index page, so we can style it differently in CSS.
  if (!htmlStr.includes("<body>")) {
    throw new Error("Error styling <body> when building the index page.");
  }
  htmlStr = htmlStr.replace("<body>", '<body id="index-page">');
  // Use a plain title without the "- Guitar Tabs" suffix.
  htmlStr = htmlStr.replace(/<title>.+?<\/title>/, "<title>Guitar Tabs</title>");
...
task("website", [], async () => {
  const files = [];
  for await (const entry of Deno.readDir("tabs")) {
    if (entry.isFile && entry.name.endsWith(".md")) {
      files.push(path.join("tabs", entry.name));
    }
  }
  files.sort();
